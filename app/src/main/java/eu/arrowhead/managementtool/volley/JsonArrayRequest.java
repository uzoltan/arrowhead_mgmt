...
//Custom volley request with JSONArray payload. The 1 header is preset.
    ...
    protected Response<JSONArray> parseNetworkResponse(NetworkResponse response) {
        try {
            ...
            if(jsonString.isEmpty()){
                ...
                * tries to create a JSONArray from the empty string payload, which causes an exception.*/
                return Response.success(new JSONArray("[{\"test\":\"value\"}]"), HttpHeaderParser.parseCacheHeaders(response));
            }
            else{
